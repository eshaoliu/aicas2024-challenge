...
static int llama_decode_prefill(
         llama_context & lctx,
           llama_batch   batch_all) { // TODO: rename back to batch
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; cur_token += n_ubatch) {
        ...
        LLAMA_LOG_ERROR("==========hparams.causal_attn:%d",hparams.causal_attn);
        // non-causal masks do not use the KV cache
        if (hparams.causal_attn) {
            llama_kv_cache_update(&lctx);
...
static void llama_kv_cache_update_internal(struct llama_context & lctx) {
    ...
    //LLAMA_LOG_ERROR("================lctx.model.hparams.rope_type != LLAMA_ROPE_TYPE_NONE:%d,lctx.kv_self.has_shift:%d\n", lctx.model.hparams.rope_type != LLAMA_ROPE_TYPE_NONE,lctx.kv_self.has_shift);
    //lctx.kv_self.has_shift =true;
    ...
    //LLAMA_LOG_ERROR("================lctx.kv_self.recurrent:%d,lctx.kv_self.do_copy:%d\n", lctx.kv_self.recurrent,lctx.kv_self.do_copy);
...
int32_t llama_decode_prefill(
        struct llama_context * ctx,
          struct llama_batch   batch) {
    const int ret = llama_decode_prefill(*ctx, batch);
    if (ret < 0) {
        LLAMA_LOG_ERROR("%s: failed to decode, ret = %d\n", __func__, ret);
    }
